...
var fetch = module.exports = function(app, query_param, mdname_param) {
	...
	var newQuery = function(session) {
	    //TODO: Shall we check for existing Query with same search_cql string?
	...
	var checkRetrievalResults = function() {
		...
		utils.removeDocument(app.query, function() { app.log.error('No records founds.') });
	}
	...
	var retrieveMDRecord = function(options, params, query, last) {
		...
		res.on('end', function () {
			...
			if(last) process.nextTick(checkRetrievalResults);
		    }
		});
	    }).on('error', function(e) {
	...
	var retrieveSRWResult = function(options, query) {
		...
		res.on('end', function() {
			...
			app.log.error("Invalid SRU/W query: " + str);
			utils.removeDocument(query);
		    }
		});
	    }).on('error', function(e) {
	...
	var parseSRWQuery = function(data, query) {
		...
		record_start += record_limit; retrieveSRWResult(utils.srw_options(search_cql, {start: record_start, limit: record_limit}, true), query); 
