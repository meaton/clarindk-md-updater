...
var fetch = module.exports = function(app, query_param, mdname_param) {
    ...
    var startSession = function() {
        ...
        app.session.users.push({
            handle: handle,
            fullname: app.config.get("fullname"),
            email: app.config.get("email")
        });

        app.log.info("Creating new Session-" + app.session.id);
    ...
    var newQuery = function(session) {
        var query = app.schema.CreateQuery({
            _session: session.id,
            search_path: search_cql
        });
    ...
    var retrieveAllMDRecords = function(query) {
        ...
        app.query = query;
        if (useCQL)
            return retrieveSRWResult(utils.srw_options(search_cql, {
                start: record_start,
                limit: record_limit
            }), query)
        else {
    ...
    var checkRetrievalResults = function() {
        if (app.query.result_collection.length <= 0)
            utils.removeDocument(app.query, function() {
        ...
        http.get(options, function(res) {
            ...
            res.on('end', function() {
                if (res.statusCode == 200)
                    utils.saveMDRecord(str, params, query, function() {
                        if (last) checkRetrievalResults();
                    });
                else
            ...
            res.on('end', function() {
                if (res.statusCode == "200")
                    ...
                    utils.removeDocument(query, function() {
                        throw new Error("Invalid SRU/W query: " + str);
                    });
            });
        }).on('error', function(e) {
            utils.removeDocument(query, function() {
    ...
    var parseSRWQuery = function(data, query) {
        ...
        for (var i = 0; i < mdRecords_res.length; i++) {
            ...
            if (name == metadataName) {
                ...
                var node_attr_href = nnode.getAttributeNS("http://www.w3.org/1999/xlink", "href"),
                    itemID = /(dkclarin\:)?([0-9]+)/.exec(node_attr_href)[0];

                if (mdrecord_data != null) // Save MdRecord if data exists
                    utils.saveMDRecord(new XMLSerializer().serializeToString(mdrecord_data), {
                        id: itemID,
                        name: metadataName
                    }, query);
            }
        ...
        if (Number(count[0].textContent) >= (record_limit + record_start)) {
            ...
            retrieveSRWResult(utils.srw_options(search_cql, {
                start: record_start,
                limit: record_limit
            }, true), query);
        } else if (count[0].textContent <= 0) {
            utils.removeDocument(query, function() {
                app.log.warn("Your CQL query returned nil results.");
                process.exit(0);
            });
        } else {
            utils.exit(0);
