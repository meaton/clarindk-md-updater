...
var remove = module.exports = function(app, queryId) {
	...
	Query.findById(queryId)
	    ...
	    if(query != null && query._session == app.session._id.toString()) { // Session match
	    	 var results = query.result_collection;
	    	 Query.update({_id: queryId}, {'$pullAll': {result_collection: results}}).exec(function(err) {
		    if(err) throw err;
		    MDRecord.remove({_id: {$in: results}}, function(err, noRemoved) {
			if(err) throw err;
			app.log.debug('Records removed: ' + noRemoved);
			utils.removeDocument(query, function() { app.log.info('Query ' + queryId + ' is now removed.'); });			 });
		 });
	    } else {
    ...
    utils.getSession(app.schema.models.Session, function(res, err) { if(!err) openSession(res, removeQuery); else throw err; });
