...
var inspect = module.exports = function(app, queryId) {
	...
	Query.findById(queryId)
		...
		if(showData)
		    prettyPrintQuery(query, function() { promptForItemID(query) });
		else
		    prettyPrintQuery(query, function() { process.exit(0); });
	    } else {
    ...
    utils.getSession(app.schema.models.Session, function(res, err) { if(!err) openSession(res, findQuery); else throw err; });
