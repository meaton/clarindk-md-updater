...
var show = module.exports = function(app, queryId) {
	...
	if(err) app.log.error(err.message.red);
  	prompt.message = "Print MD-Record data".magenta;
   	prompt.delimiter = " >> ";
 	...
 	prompt.start();
  	...
  	prompt.get({
   	    properties: {
     		itemID: {
        	description: "Enter DKCLARIN-ID".green
     	    	}
   	    }
    	}, function (err, result) {
	    ...
	    if(findResult) prettyjson(findResult, function() { promptForItemID(query); });
	    else promptForItemID(query, new Error("Could not find result with ID: " + result.itemID));
