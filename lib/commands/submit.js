 ...
 *
 */
var utils = require('../utils'), fs = require('fs'), util = utils.util, http = require('http'), parser = require('xml2json');
		 ...
		 .exec(function(err, query) {
	   	...
	   	if(query != null && query._session == app.session._id.toString() && !query.complete) {                    
			...
			// change-rules previously applied by `update` cmd 
		        ...
		        // Previously sanitized element data in update
		       	    var data = parser.toXml(d);
			
			    app.log.debug("JSONtoXML escape output: " + data);
