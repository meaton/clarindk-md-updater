 ...
 *
 */
var utils = require('../utils'), fs = require('fs'), util = utils.util, http = require('http'), parser = require('xml2json'), jsontoxml = require('jsontoxml');
var submit = module.exports = function(app, queryId, comment) {
	...
	var result = parser.toJson(data, {object: true, sanitize: false});
	
	submission.emit("submit", record, result['result']['last-modification-date']);	
    });
	...
	Query.findById(queryId).populate('_session', null, {_id: {$in: [app.session._id]}})
	     .populate("result_collection", null, {"status.completed":false, "status.updated": true})
	     .where("results_collection").size(0).exec(function(err, query) { 
	...
	batchList.push({data: data, record: record});
   ...
   var updateMDRecordCallback = function(res, res_data, data, record) {
	   ...
	   app.log.debug('Response code: ' + res.statusCode);
	   if(app.batch && res.statusCode == 400)
		 ...
		 .exec(function(err, query) {
	   	...
	   	if(query != null && query._session == app.session._id.toString() && !query.complete) {                    
			...
			// change-rules previously applied by `update` cmd 
		        ...
		        // Add sanitize hack to escape illegal chars
		       	    
			    var sanitize = function(key, val) {
			        if(typeof(val) == "string" && key == "$t")
			            return jsontoxml.escape(val);
			        return val;
			    };
		
			    var data_san = JSON.stringify(d, sanitize);
    ...
    utils.getSession(Session, function(res, err) { if(!err) openSession(res, readDatastore); else throw err; });
