...
var submit = module.exports = function(app, queryId, comment) {
    ...
    submission.on('update', function(record, data) {
    	...
    	//app.log.debug("ID received: " + record.dkclarinID);
    	//app.log.debug("response data: " + data); // Comment: Should data in future contains lastModificationDate of Item version we wont need this version-history call step
	...
	// Lookup for any results with completed:false
	...
	if(app.session != null) {
	    Query.findById(queryId).populate('_session', null, {_id: {$in: [app.session._id]}}).populate("result_collection").exec(function(err, query) {
	   	...
	   	if(query != null && !query.complete) {                    
			...
			// change-rules previously applied by `update` cmd 
		        // fk. change d['olac:olac']['dc:subject']['$t'] = "unspecified";
		        // Get data and convert back to XML for transfer
		       	    var data = parser.toXml(d); 
	   	    	    app.log.info("Updating local record " + record._id + " for item " + record.dkclarinID);
			    //app.log.debug("Submitting comment: " + comment);
