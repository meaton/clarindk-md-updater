...
var submit = module.exports = function(app, queryId, comment) {
    ...
    app.batch = (app.config.get('useBatch') == true);
    ...
    submission.on('update', function(record, data) {
        var restresp_parser = new xml2js.Parser({ mergeAttrs: true });
        ...
        data = data + "</result>"; //root element correction

        restresp_parser.parseString(data, function(err, result) {
          //app.log.info(util.inspect(result, false, null));
          if (!err) submission.emit("submit", record, result['result']['last-modification-date'][0]);
          else app.log.error("Failed handing update response: " + err);
        });
    });
            ...
            .exec(function(err, query) {
                ...
                if (query != null && query.result_collection.length <= 0) {
                    ...
                    utils.saveDocument(query, function(err) {
                        utils.exit(0);
                        app.log.info('Query +' + query.id + ' marked as completed.');
                        console.log('Exiting..');
                    });
    ...
    var updateMDRecord = function(data, record) {
        if (!record.complete && record.reversible) {
            var params = {
            ...
            if (record.complete) app.log.warn("MD-Record already marked as completed: " + record.id);
            if (!record.reversible) app.log.error("Cannot submit MD-Record because a non-default parser configuration has been used.");
    ...
    var readDatastore = function(queryId) {
        ...
        app.log.debug('batch: ' + app.batch);
        app.log.debug('batchLimit: ' + app.batchLimit);
        ...
        if (app.session != null)
            ...
            .exec(function(err, query) {
                if (err) console.error(err);
                if (query != null && query._session.id == app.session.id.toString() && !query.complete) {
                    ...
                    results.forEach(function(result) {
                        ...
                        if (record != null && record.data != null) {
                            ...
                            app.log.info("Updating local record " + record.id + " for item " + record.dkclarinID);
                            if (!app.batch) {
                            ...
                            if (record != null) app.log.error("Empty record data for MD-Record:" + record.id);
                            else app.log.error("No results found for Query:" + query.id);
                        }
                    });
                    ...
                    if (!query || query._session != app.session.id.toString())
                        throw new Error("Could not find query for session_id: " + app.session.id);
                    else
	...
	Query.findOne({"search_ids": {$in: [id]}}).populate("_session", null, {_id: {$in: [session.id]}}).populate("result_collection").exec(function(err, query) {
	    if(query != null) {
		var results = query.result_collection;
