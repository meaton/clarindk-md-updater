...
var addUser = module.exports = function(app, _handle) {
	...
	if(err) throw err;
	if(session != null)
	    throw new Error("User with same handle already belongs to Session: " + session._id);	
	else {
	...
	if(app.colors.mode == "none") 
   	...
   	prompt.delimiter = " >> ";
	var description1 = "fullname (value: " + app.config.get("fullname") + "): ";
	var description2 = "email (value: " + app.config.get("email") + "): ";
	...
	app.config.save(function() { app.log.info("Config set."); utils.exit(0); } );
    }

    // check Session exists for User(escidoc_handle)?
    utils.getSession(Session, function(res, err) { if(!err) openSession(res, addNewUser); else throw err; });
