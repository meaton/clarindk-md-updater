...
var addUser = module.exports = function(app, _handle) {
    ...
    var confirmNewUser = function(err, session) {
        if (err) throw err;
        if (session != null)
            throw new Error("User with same handle already belongs to Session: " + session.id);
        else {
            confirmNameEmail(function(_fullname, _email) {
                // add new User and save Session
                app.log.info("Saving new User in Session: " + app.session.id);
    ...
    utils.getSession(Session, function(res, err) {
        if (!err) openSession(res, addNewUser);
        else throw err;
    });
