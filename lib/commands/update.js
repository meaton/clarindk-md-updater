...
var update = module.exports = function(app, queryId, change_json_file) {
    ...
    var attrkey = utils.getParserConfig().attrkey;
    var charkey = utils.getParserConfig().charkey;
            ...
            if (cond_pair != null && util.isArray(cond_pair)) {
                for (var i = 0; i < cond_pair.length; i++) {
                    ...
                    if (field != undefined && val != undefined)
                        if (field == "$match")
                            match = val;
                        else if ((field == charkey && data[field] != val) || (field != charkey && data[attrkey][field] != val))
                            return null; //unchanged value if condition doesn't match
        ...
        findFieldPosition(d, rule.field_position, rule.selector, false, newAttr, function(data) {
            ...
            if (cond_pair != null && util.isArray(cond_pair)) {
                for (var i = 0; i < cond_pair.length; i++) {
                    field = cond_pair[i][0], val = cond_pair[i][1];
                    if (field != undefined && val != undefined) {
                        if ((field === charkey && data[field] != val) || (field != charkey && data[attrkey][field] != val)) {
                             app.log.warn('Conditional match not found on selected field: ' + field + ' ' + val);
                             return null; //unchanged value if condition doesn't match
                        } else app.log.info('Conditional match found on selected field: ' + field);
                    }
                }
            }
            ...
            if (util.isArray(data[target])) {
                if (createNewFields) {
                    ...
                    for (var i = 0; i < data[target].length; i++) {
                      //app.log.warn('Testing array position [' + i + '] on ' + target);
                      findFieldPosition(data[target][i], null, selector, createNewFields, createNewAttr, callback);
                    }
                }
            } else if (target != null && data[target] != undefined) {
                if (createNewFields)
              ...
              if (cb != null) { // TODO: test against callback function
                if (data[selector] != undefined || selector === charkey) {
                    data[selector] = cb;
                } else if ((data[attrkey][selector] != undefined) || (data[attrkey][selector] == undefined && createNewAttr)) {
                    data[attrkey][selector] = cb;
                } else if (selector != null) {
                    app.log.error("Invalid change-rule syntax: Invalid selector: " + selector);
                }

                if (createNewFields && data[target] == undefined) { // No existing field with name exists
                    data[target] = cb;
                } else if (target != null) {
                    app.log.error("Invalid change-rule syntax: Invalid target: " + target);
                }
              }
            }
        } else {
            _u.each(target, function(val, key, list) {
                if (data[key] != undefined) findFieldPosition(data[key], val, selector, createNewFields, createNewAttr, callback);
                else if (data[selector] != undefined) data[selector] = callback(data);
                else if (data[attrkey][selector] != undefined) data[attrkey][selector] = callback(data);
                else if (createNewFields) data[target] = callback(new Object());
                else if (createNewAttr) data[attrkey][selector] = callback(data);
                else {
