...
var update = module.exports = function(app, queryId, change_json_file) {
    ...
    var accepted_change_types = { override: "override", addition: "addition", replace: "replace"};
	...
	app.schema.models.Query.findById(queryId).populate({path:'_session', select:'_id'}).populate("result_collection", null, {"status.completed":false}).exec(function(err, query) {
	    if(err) throw err;
	    if(query != null && query._session.id == app.session._id.toString()) {
	        Query = query;
	...
	switch(rule.change_type) {
	    ...
	    case accepted_change_types.replace: applyChangeReplace(mdrecord, rule); break;
