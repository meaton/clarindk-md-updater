...
var update = module.exports = function(app, queryId, change_json_file) {
	...
	app.schema.models.Query.findById(queryId).populate({path:'_session', select:'_id'}).populate("result_collection", null, {"status.updated":false}).exec(function(err, query) {
	    if(err) throw err;
