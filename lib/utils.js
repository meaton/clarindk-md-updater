var app = require('./app'), http = require('http'), parser = require('xml2json'), jsontoxml = require('jsontoxml'), util = require('util'), path = require('path'), error = app.log.error;
...
module.exports = {	
	saveMDRecord: function(record, params, query, callback) {
	    var doc = parser.toJson(record, {reversible: true, coerce: false, trim: false, sanitize: false});    
	...
	updateMDRecord: function(new_data, mdrecord) {
	    ...
	    var new_data_str = JSON.stringify(new_data, sanitize);
	...
	execItemREST: function(record, method, emitter, nextEvent, lastModificationDate, handle) {
		...
		res.on('end', function () {
			...
			if(res.statusCode == 503) {
			    setTimeout(function(emitter, method, record, date) { return emitter.emit(method, record, date);  }, 300000, emitter, method, record, lastModificationDate);
