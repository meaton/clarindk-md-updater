...
module.exports = {
	...
	rest_options: function(params, isUpload) {
	    return { 
		host: (app.config.get('updaterHost') != null) ? app.config.get('updaterHost') : 'localhost', //Apply config app.config.get('updaterHost'), default: localhost
		port: (app.config.get('updaterPort') != null) ? app.config.get('updaterPort') : '80', //TODO Apply config app.config.get('updaterPort'), default 80
	...
	srw_options: function(queryCQL, record_vals) {
	    ...
	    return { 
		host: (targetUrl.indexOf('http://') != -1) ? targetUrl.replace('http://', '') : targetUrl, //Apply config app.config.get('targetUrl') strip http://,
