...
module.exports = {
	...
	rest_options: function(params) {
	    return { 
		host: 'localhost', //TODO Apply config app.config.get('updaterHost'),
		port: '8082', //TODO Apply config app.config.get('updaterPort'),
	...
	srw_options: function(queryCQL, record_vals) {
	    ...
	    return { 
		host: 'devtools.clarin.dk', //TODO Apply config app.config.get('targetUrl') strip http://,
		path: '/srw/search/escidoc_all?query='+queryCQL+search_path_limits+'&d='+Date.now()
	    };
	},
	...
	getSession: function(sessionModel, callback) {
	    ...
	    if(handle == null) return null; //TODO Config check, warning
	...
	updateMDRecordREST: function(data, params, callback) {
	    //TODO: Replace with duplicate utils.rest_options(params) to handle PUT request and data
	    data = data.replace(/^\uFEFF/, ''); // Strip potential UTF-8 BOM
	    var options = {
		host: 'localhost', //TODO: config
		port: '8082', //TODO: config
		path: '/v0.9/items/'+params.id+'/metadata/'+params.name+'?escidocurl='+params.target+'&d='+Date.now(),
