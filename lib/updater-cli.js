...
app.started = false;
app.config.file({
    file: 'config.json' //path.join(__dirname, '..', 'config.json')
});
...
app.start = function(callback) {
    ...
    app.init(function(err) {
        ...
        if (app.config.get('debug') == true) app.log.loggers.default.transports.console.level = "debug";
...
app.setup = function(callback) {
    ...
    app.cmd(/addUser (ESCIDOC-[0-9a-zA-Z]{23})/, commands.addUser);
    app.cmd(/remove \b([0-9A-Fa-f]{1,24})\b/, commands.remove);
