...
var Session, User, Item, MDRecord, Query,
    Schema = mg.Schema, Mixed = Schema.Types.Mixed;

var def_metadata_sch = new Schema({
        dkclarinID: {type: String, required: true},
	type: {
...
var def_query_sch = new Schema({
        _session: {type: Schema.ObjectId, ref: "Session", required: true},        
        search_path: {type: String},
        search_ids: [String],
        result_collection: [{type: Schema.ObjectId, ref: "MDRecord"}],
	status: { 
		updated: {type: Boolean, default: false}, 
		completed: {type: Boolean, default: false},
		lastUpdatedDate: {type: Date}, 
    ...
    def_user_sch.method('userHandleToBase64', function() { return new Buffer(this.handle).toString("base64"); });
    
User = mg.model('User', def_user_sch);
var def_session_sch = new Schema({
        created_on: {type: Date, default: Date.now},
        expires_on: {type: Date}, //TODO Set expiration based on handle
...
def_session_sch.static('findByHandle', function(handle, callback) {
	return this.findOne({ "users.handle": handle}, callback);
    });
...
var QueryModel = function() {
	...
	.get(function() { return this.status.updated; })
	.set(function(v) { if(v) { this.set('status.updated', v); this.set('status.lastUpdatedDate', new Date()); } });
    def_query_sch.virtual('complete')
	.get(function() { return this.status.completed; })
	.set(function(v) { if(v) { this.set('status.completed', v); this.set('status.completedDate', new Date()); } });
    def_query_sch.virtual('results.count').get(function() { return this.result_collection.length; });
